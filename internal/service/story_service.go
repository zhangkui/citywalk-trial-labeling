...
func (s *StoryService) Create(ctx context.Context, title, content, coverImage string, landmarkIDs []int64, routeID *int64, createdBy int64, media []map[string]any) (map[string]any, error) {
	...
	for _, item := range normalizeStoryMedia(media) {
		url, _ := item["url"].(string)
		typ, _ := item["type"].(int8)
		order, _ := item["order"].(int)
		if url != "" {
			_ = s.c.Deps.Store.AddStoryMedia(ctx, id, typ, url, order)
		}
	}
...
func (s *StoryService) Update(ctx context.Context, id int64, title, content, coverImage string, landmarkIDs []int64, routeID *int64, status int8, media []map[string]any) error {
	...
	// The story row and its media are swapped in one transaction so a failure
	// while saving any media item rolls back the body edit and the deletion of
	// the old media. The whole edit either commits or has no effect.
	return s.c.Deps.Store.UpdateStoryWithMedia(ctx, id, title, content, coverImage, landmarkIDs, routeID, status, normalizeStoryMedia(media))
}

// normalizeStoryMedia coerces each client-supplied media item into a stable
// shape (typed int8 type, trimmed url, explicit order) and assigns a position
// to items that did not request one, preserving the request order for the rest.
func normalizeStoryMedia(media []map[string]any) []map[string]any {
	...
	for idx, item := range media {
		...
		if value, ok := item["type"].(float64); ok {
			typ = int8(value)
		} else if value, ok := item["type"].(int); ok {
			typ = int8(value)
		} else if value, ok := item["type"].(int8); ok {
			typ = value
		}
		order := idx
		if value, ok := item["order"].(float64); ok {
			order = int(value)
		} else if value, ok := item["order"].(int); ok {
			order = value
		}
		result = append(result, map[string]any{"type": typ, "url": strings.TrimSpace(url), "order": order})
	}
	return result
}
